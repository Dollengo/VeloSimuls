...
void menu(Player &player) {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1: player.moveUp(); break;       // Move player up
            case 2: player.moveDown(); break;     // Move player down
            case 3: player.moveRight(); break;    // Move player right
            case 4: player.moveLeft(); break;     // Move player left
            case 5: return;                       // Exit the game loop
        ...
        if (x >= GRID_WIDTH) player.setPosition(GRID_WIDTH - 1, y);
        if (y < 0) player.setPosition(x, 0);
        if (y >= GRID_HEIGHT) player.setPosition(x, GRID_HEIGHT - 1);
...
int main() {
    ...
    std::cout << "Welcome to VeloSimul! A minigame to simulate physics (I think...)\n";
    std::cout << "Please enter the Player velocity: ";
    std::cin >> velocity;

    std::cout << "Please enter the start player X position (0 to " << GRID_WIDTH - 1 << "): ";
    std::cin >> startX;

    std::cout << "Please enter the start player Y position (0 to " << GRID_HEIGHT - 1 << "): ";
    std::cin >> startY;

    // Ensure starting positions are within grid boundaries
    if (startX < 0) startX = 0;
    if (startX >= GRID_WIDTH) startX = GRID_WIDTH - 1;
